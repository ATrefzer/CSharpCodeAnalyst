...
**Tests** are in `Tests/UnitTests/Mcp/`. The tool fixtures call the methods directly against a `TestCodeGraph` and pin the promises the descriptions make; `McpServerHostTests` covers transport, routing and serialization end to end over a real socket on an OS-assigned free port.
