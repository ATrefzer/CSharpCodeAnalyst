...
- **`CSharpCodeAnalyst.Importers/`** — every import except the C# solution: doxygen (C++/Python/Java), Dart/Flutter, jdeps (Java, from bytecode) and the plain text format. UI-free apart from each importer's own configuration dialog; references only `CodeGraph` and `AnalyzerSdk`, never the Roslyn parser. See **Importers (how to add one)** below.
- **`CSharpCodeAnalyst.Mcp/`** — the optional MCP server: a loopback HTTP endpoint that lets an AI assistant query the loaded graph (`McpServerHost`), plus the tools it exposes (`Tools/`) and the snapshot contract the application fills (`Contracts/`). References only `CodeGraph`, no WPF, so the tools are unit-tested against a hand-built graph. See **MCP server** below and `Documentation/mcp.md`.
...
The server is opened from **Home → MCP → Start MCP server**; nothing listens until someone presses it. `Features/Mcp/McpServerService` owns the lifetime, `Features/Mcp/CodeGraphSnapshotProvider` supplies the data, and everything else lives in `CSharpCodeAnalyst.Mcp/`.

**No ASP.NET, and that is load-bearing.** The package is `ModelContextProtocol.Core`, the endpoint is a `System.Net.HttpListener`. The obvious host would be Kestrel via `ModelContextProtocol.AspNetCore`, but that package carries a `FrameworkReference` to `Microsoft.AspNetCore.App` which propagates through the `ProjectReference` into the **application's** `runtimeconfig.json` — and the .NET host refuses to start a process whose declared frameworks are not all installed. A missing ASP.NET runtime then costs the whole application, not just the MCP feature. Do not pull the ASP.NET package back in, not even transitively; check `runtimeconfig.json` after touching the package references.

**The tool descriptions are the entire interface.** A model never sees the code, only the `[Description]` attributes: the one on the method becomes the tool description, the ones on parameters become their JSON-schema descriptions. **`///` XML comments do not travel** — no documentation file is generated, and the SDK reads the attribute. A tool method without `[Description]` ships with an empty description; nothing fails to compile and no test goes red, it simply never gets chosen. Write the XML comments for developers and the attribute for the model, and keep `Documentation/mcp.md` in sync with both.

**Answers are prose for a model, not serialized objects** (`Tools/ToolText.cs` explains why). Two habits matter more than the formatting: say what to do next rather than only what is missing, and never let an absence read as a finding — an empty result that could be misread has to state what it does not prove. The heuristic warning in `find_incoming_calls` and the "this element is external" note are the reference examples. Truncation always says how many entries were dropped and how to reach them.

**Stateless, and answering from a copy.** Every request stands on its own; `CodeGraphSnapshotProvider` clones the graph on the UI thread and hands out that snapshot, so tool calls never touch the live graph. Element ids are only valid while the server runs.

**The accept loop must not run on the UI thread.** `StartAsync` is called from the button handler, so it hands the loop to the thread pool with `Task.Run` and every `await` in the host uses `ConfigureAwait(false)`. Without that, the first `await` captures the dispatcher and accepting connections, parsing request bodies and closing responses all move onto the thread that draws the application — Kestrel used to bring its own threads and hid this. `ServingARequest_NeverReturnsToTheCallersThread` pins it, because the symptom (a UI that stutters while an assistant is asking questions) is not one anybody would trace back here.

**Re-importing while the server is running just works — no reconnect needed.** `LoadCodeGraph` calls `CodeGraphSnapshotProvider.SetGraph` on every import, load and restore regardless of whether the server is up, and the next tool call picks up the new graph on its own; the socket, the tool list and the client connection are untouched. This is a different situation from the *tool list* changing (new tool, changed description), which genuinely does need `/mcp` → reconnect — see `Documentation/mcp.md`. Don't conflate the two when explaining this to a user.

`CodeGraphSnapshotProvider.SetGraph` and an in-flight capture can genuinely race: `Capture()` clones the graph on the UI thread, which takes real time for anything but a trivial graph, so a re-import can land while a previous capture is still copying. Both the capture's commit and the new `SetGraph`'s "mark stale" take the same lock, and whichever runs last wins — if that is the capture, it stamps `_isStale = false` over a snapshot of the graph as it was *before* the swap, and the server keeps answering from the old project until an unrelated later change fixes it by accident. The fix is a generation stamp compared at commit time: a `SetGraph` that happened mid-capture already moved the generation on, so the capture's commit is skipped instead of overwriting it. `CodeGraphSnapshotProviderTests.ALostUpdateBetweenSetGraphAndAnInFlightCapture_MustNotResurrectTheOldSnapshot` reproduces it against a real `Dispatcher` (roughly 80% of iterations hit it without the generation check) and pins the fix at zero.

**Adding a tool:** a public method on one of the `[McpServerToolType]` classes in `Tools/`, carrying `[McpServerTool(Name = "…")]` and `[Description]`. Set `Name` explicitly — otherwise the SDK derives it from the method name, and renaming the method would silently change the wire contract. Adding a method to an existing class needs no registration; adding a whole new tool **class** means adding it to the list in `McpServerHost.CreateOptions`.

**Tests** are in `Tests/UnitTests/Mcp/`. The tool fixtures call the methods directly against a `TestCodeGraph` and pin the promises the descriptions make; `McpServerHostTests` covers transport, routing and serialization end to end over a real socket on an OS-assigned free port; `CodeGraphSnapshotProviderTests` covers the UI-thread/thread-pool boundary, including the lost-update race above.
...
**The reader's guide and the upstream record are two different documents — keep them apart.** The reader's guide is not in this folder: it is **Appendix A of `Documentation/dsm-tutorial.md`**, which describes what you see and how to operate it (axes — row = provider, column = consumer; the depth-coloured blocks; the row indicator bar; the presence map when zoomed out; keyboard and wheel) for an audience that does not read code. `ThirdParty/DsmSuite/README.md` is the record against upstream: the change table, why each change was made, and the bug list. Read that appendix before changing anything about the view's semantics and keep it in sync — none of it is discoverable from the UI — but put the reasoning in `README.md`.
